...
xlabel('k');
legend('y', 'yZad')
saveas(f, sprintf('./lambda10/N/N_Wyjscie_D=%d_N=%d_Nu=%d.jpg', D, N, Nu));
close(f);
...
xlabel('k');
legend('u')
saveas(f, sprintf('./lambda10/N/N_Sterowanie_D=%d_N=%d_Nu=%d.jpg', D, N, Nu));
close(f);
end
...
xlabel('N');
legend('Jy')
saveas(f, sprintf('./lambda10/N/N_Jy_D=%d_N=%d_Nu=%d.jpg', D, N, Nu));
close(f);
f = figure;
plot(x, errorJU);
xlabel('N');
legend('Ju')
saveas(f, sprintf('./lambda10/N/N_Ju_D=%d_N=%d_Nu=%d.jpg', D, N, Nu));
close(f);
